import type { InventoryFilters, InventoryItem, PaginatedResponse, PaginationQuery, PdfTemplate, User } from "../types";
...
export function getUsers(pagination: PaginationQuery): Promise<PaginatedResponse<User>> {
  const params = toSearchParams({}, pagination);
  return request<PaginatedResponse<User>>(`${apiEndpoints.users}?${params}`);
}
...
export function getInventoryItems(
  ...
  const params = toSearchParams(filters, pagination);
  return request<PaginatedResponse<InventoryItem>>(`${apiEndpoints.inventoryItems}?${params}`);
}
...
export async function exportInventoryPdf(filters: InventoryFilters, template: PdfTemplate): Promise<void> {
  const params = toSearchParams(filters, undefined, template);
